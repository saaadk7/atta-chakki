...
//       flourSummary[txn.flourType].totalPrice += parseFloat(txn.quantity) * parseFloat(txn.unitPrice);
//       flourSummary[txn.flourType].electricityUnits += Math.floor(parseFloat(txn.quantity) / 5); // 5kg = 1 unit
...
//       weeklyData[weekNumber].totalAmount += parseFloat(txn.quantity) * parseFloat(txn.unitPrice);
...
//       totalPrice: flourSummary[flourType].totalPrice,
...
//     if (weeks.length > 0) {
//       setCurrentWeek(weeks[0].weekNumber); // Default to most recent week
//     }
//   };
...
//       head: [["ID", "Customer", "Flour Type", "Date", "Qty (kg)", "Unit Price", "Total"]],
...
//       flourSummaryForWeek[txn.flourType].totalPrice += parseFloat(txn.quantity) * parseFloat(txn.unitPrice);
//       flourSummaryForWeek[txn.flourType].electricityUnits += Math.floor(parseFloat(txn.quantity) / 5);
//     });

//     doc.autoTable({
//       startY: doc.lastAutoTable.finalY + 15,
//       head: [["Flour Type", "Qty (kg)", "Total Amount", "Electricity Units"]],
//       body: Object.keys(flourSummaryForWeek).map(flourType => [
//         flourType,
//         flourSummaryForWeek[flourType].quantity,
//         formatCurrency(flourSummaryForWeek[flourType].totalPrice),
...
//                       <td className="py-2 px-4 border">{formatCurrency(item.totalPrice)}</td>
...
//               </td>
//               <td className="py-2 px-4 border">
//                 {formatCurrency(flourSummary.reduce((sum, item) => sum + item.totalPrice, 0))}
//               </td>
//               <td className="py-2 px-4 border">
