...
const TransactionReport = () => {
  ...
  const calculateSummaries = (transactions) => {
    ...
    transactions.forEach((txn) => {
      ...
      const transactionAmount =
        parseFloat(txn.quantity) * parseFloat(txn.unitPrice);
      ...
      flourSummary[txn.flourType].electricityUnits += Math.floor(
        parseFloat(txn.quantity) / 5
      );
      ...
      weeklyData[weekNumber].totalElectricity += Math.floor(
        parseFloat(txn.quantity) / 5
      );
    });

    setFlourSummary(
      Object.keys(flourSummary).map((flourType) => ({
        flourType,
  ...
  const handleDownloadWeeklyPDF = (week) => {
    ...
    doc.setFontSize(12);
    doc.text(
      `Period: ${formatDate(week.startDate)} to ${formatDate(week.endDate)}`,
      14,
      22
    );
    ...
    doc.autoTable({
      ...
      body: [
        ...
        [
          ...
          `${week.totalElectricity} units (5kg = 1 unit)`,
        ],
      ],
      styles: { fontSize: 12 },
    ...
    week.transactions.forEach((txn) => {
      ...
      flourSummaryForWeek[txn.flourType].electricityUnits += Math.floor(
        parseFloat(txn.quantity) / 5
      );
    });
    ...
    doc.save(
      `Weekly-Report-Week-${week.weekNumber}-${formatDate(
        week.startDate
      )}-to-${formatDate(week.endDate)}.pdf`
    );
  };
  ...
  const selectedWeek =
    weeklySummary.find((w) => w.weekNumber === currentWeek) || {};
        ...
        <div className="mb-8 p-4 bg-gray-50 rounded-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
            ...
            <div className="bg-white p-4 rounded shadow">
              ...
              <p className="text-2xl font-bold">
                {selectedWeek.totalElectricity} units
              </p>
          ...
          <div className="mt-8">
            <h3 className="text-lg font-semibold mb-4">
              Flour Type Summary (Week {selectedWeek.weekNumber})
            </h3>
            <table className="min-w-full bg-white border">
              ...
              <tbody>
                {flourSummary
                  .filter((item) =>
                    selectedWeek.transactions.some(
                      (txn) => txn.flourType === item.flourType
                    )
                  )
                  .map((item, index) => (
                    <tr key={index}>
        ...
        <table className="min-w-full bg-white border">
          ...
          <tbody>
            ...
            <tr className="font-bold">
              ...
              <td className="py-2 px-4 border">
                {formatCurrency(
                  flourSummary.reduce((sum, item) => sum + item.totalAmount, 0)
                )}
              </td>
              <td className="py-2 px-4 border text-green-600">
                {formatCurrency(
                  flourSummary.reduce((sum, item) => sum + item.revenue, 0)
                )}
              </td>
              <td className="py-2 px-4 border">
                {flourSummary.reduce(
                  (sum, item) => sum + item.electricityUnits,
                  0
                )}
              </td>
            </tr>
